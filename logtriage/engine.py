...
def analyze_file(file_path: Path, pcfg: PipelineConfig) -> List[Finding]:
    try:
    ...
    findings = classify_lines(pcfg, file_path, pcfg.name, lines, 1)
    for f in findings:
        f.needs_llm = should_send_to_llm(pcfg, f.severity, f.excerpt)
    return findings


def analyze_path(root: Path, pipelines: List[PipelineConfig], pipeline_override: Optional[str] = None) -> List[Finding]:
